...
export function AdminDashboard({ user, onNavigateToHome, onLogout, activeTab = 'dashboard', onTabChange }: AdminDashboardProps) {
  ...
  const [realUsers, setRealUsers] = useState<PlatformUser[]>([]);
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        setRealUsers([]); // Set empty array on error
      } finally {
        setLoading(false);
      }
    };
  ...
  const renderTabContent = () => {
    switch (currentTab) {
      case 'users':
        return <UserManagement users={realUsers} loading={loading} />;
      case 'artists':
        return <ArtistVerification users={realUsers.filter(u => u.role === 'artist')} loading={loading} />;
      case 'artwork':
...
function UserManagement({ users, loading }: { users: PlatformUser[]; loading: boolean }) {
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-12">
                  <div className="flex flex-col items-center">
                    <Users className="w-12 h-12 text-stone-400 mb-4" />
...
function ArtistVerification({ users, loading }: { users: PlatformUser[]; loading: boolean }) {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <Card className="p-12 bg-white/80 backdrop-blur-sm border-stone-200/50 text-center">
          <div className="animate-spin w-8 h-8 border-4 border-purple-600 border-t-transparent rounded-full mx-auto mb-4"></div>
          <p className="text-stone-600">Loading artists...</p>
        </Card>
      </div>
    );
  }
        ...
        {pendingArtists.length === 0 && (
          <Card className="p-12 bg-white/80 backdrop-blur-sm border-stone-200/50 text-center">
            ...
            <p className="text-stone-600">
              {users.length === 0 ? 'No artists found in the database' : 'No pending artist verifications'}
            </p>
            {users.length === 0 && (
              <p className="text-sm text-stone-500 mt-2">
                Artists will appear here once they register and upload artwork.
              </p>
            )}
          </Card>
        )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6 bg-white/80 backdrop-blur-sm border-stone-200/50">
          ...
          <div className="h-64 bg-gradient-to-br from-stone-50 to-neutral-50 rounded-lg p-4">
            <svg viewBox="0 0 400 200" className="w-full h-full">
              ...
              {[0, 0.25, 0.5, 0.75, 1].map((fraction, index) => (
                <line key={index} x1="0" y1={fraction * 180} x2="400" y2={fraction * 180} stroke="#e5e7eb" strokeWidth="1"/>
              ))}
              {/* Revenue bars */}
              {[
                { month: 'Jan', value: 180000, height: 0.6 },
                { month: 'Feb', value: 220000, height: 0.73 },
                { month: 'Mar', value: 195000, height: 0.65 },
                { month: 'Apr', value: 280000, height: 0.93 },
                { month: 'May', value: 320000, height: 1.0 },
                { month: 'Jun', value: 290000, height: 0.97 }
              ].map((item, index) => (
                <g key={item.month}>
                  <rect
                    x={60 + index * 50}
                    y={180 - item.height * 180}
                    width="30"
                    height={item.height * 180}
                    fill="url(#revenueGradient)"
                    rx="4"
                  />
                  <text x={75 + index * 50} y="195" textAnchor="middle" className="text-xs fill-gray-600" fontSize="10">
                    {item.month}
                  </text>
                </g>
              ))}
        ...
        <Card className="p-6 bg-white/80 backdrop-blur-sm border-stone-200/50">
          ...
          <div className="h-64 bg-gradient-to-br from-stone-50 to-neutral-50 rounded-lg p-4">
            <svg viewBox="0 0 400 200" className="w-full h-full">
              ...
              {/* User growth line */}
              <path
                d="M 40,160 L 80,140 L 120,120 L 160,100 L 200,85 L 240,75 L 280,70 L 320,65 L 360,60"
                stroke="#3b82f6"
                strokeWidth="3"
                fill="none"
                strokeLinecap="round"
              ...
              {/* Area under the line */}
              <path
                d="M 40,160 L 80,140 L 120,120 L 160,100 L 200,85 L 240,75 L 280,70 L 320,65 L 360,60 L 360,180 L 40,180 Z"
                fill="url(#growthGradient)"
              />
              {/* Data points */}
              {[
                { month: 'Jan', users: 1200, x: 40, y: 160 },
                { month: 'Feb', users: 1400, x: 80, y: 140 },
                { month: 'Mar', users: 1600, x: 120, y: 120 },
                { month: 'Apr', users: 1800, x: 160, y: 100 },
                { month: 'May', users: 1950, x: 200, y: 85 },
                { month: 'Jun', users: 2100, x: 240, y: 75 },
                { month: 'Jul', users: 2200, x: 280, y: 70 },
                { month: 'Aug', users: 2300, x: 320, y: 65 },
                { month: 'Sep', users: 2400, x: 360, y: 60 }
              ].map((point) => (
                <g key={point.month}>
                  <circle cx={point.x} cy={point.y} r="4" fill="#3b82f6"/>
                  <text x={point.x} y="195" textAnchor="middle" className="text-xs fill-gray-600" fontSize="10">
                    {point.month}
                  </text>
                </g>
              ))}
