...
export function DashboardHeader({ userName = 'User', userRole = 'buyer', onNavigateToHome, onLogout }: DashboardHeaderProps) {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      // For artists, search their own artworks
      if (isArtist) {
        // Navigate to artist dashboard with search results
        navigate('/artist-dashboard?search=' + encodeURIComponent(query));
      } else {
        // For buyers and admins, navigate to browse artwork with search
        navigate('/buyer-dashboard?search=' + encodeURIComponent(query));
      }
    } catch (error) {
