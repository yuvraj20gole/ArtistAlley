...
export function ArtistDashboard({ user, onNavigateToHome, onLogout, activeTab = 'dashboard', onTabChange }: ArtistDashboardProps) {
  ...
  useEffect(() => {
    ...
    if (searchParam) {
      ...
      // Switch to my artworks tab when searching
  ...
  // Mock sales data for the chart - Realistic ups and downs from Jan to Sep
  const salesData: SalesData[] = [
    { month: "Jan", sales: 18500 },
    { month: "Feb", sales: 14200 },
    { month: "Mar", sales: 22800 },
    { month: "Apr", sales: 19600 },
    { month: "May", sales: 31200 },
    { month: "Jun", sales: 27400 },
    { month: "Jul", sales: 35800 },
    { month: "Aug", sales: 28600 },
    { month: "Sep", sales: 42100 }
  ];
  ...
  const renderTabContent = () => {
    switch (currentTab) {
      case 'artworks':
        return <MyArtworks initialSearchQuery={searchQuery} />;
      case 'orders':
        return <Orders />;
      case 'analytics':
        return <Analytics />;
        ...
        <div className="lg:col-span-2">
          <Card className="border-primary/20 bg-gradient-to-br from-card to-primary/5 h-full">
            <CardHeader>
              ...
              <CardDescription>Track your artistic success from January to September</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="relative h-96 p-4">
                  ...
                  <div className="absolute inset-4 pointer-events-none">
                    ...
                    {[0, 0.2, 0.4, 0.6, 0.8, 1].map((fraction, index) => (
                      <div
                        key={`h-grid-${index}`}
                        className={`absolute w-full border-t ${index === 0 || index === 5 ? 'border-purple-300/50' : 'border-purple-200/30'}`}
                    ...
                    {salesData.map((_, index) => (
                      <div
                        key={`v-grid-${index}`}
                        className="absolute h-full border-l border-purple-200/15"
                  ...
                  {/* Enhanced trend line connecting the bar tops */}
                      ...
                      return (
                        <g key={`point-${index}`}>
                          ...
                          <circle
                            ...
                            stroke="white"
                            strokeWidth="3"
                            className="drop-shadow-lg hover:r-7 transition-all duration-300 cursor-pointer"
                            filter="url(#glow)"
                          />
                        </g>
                      );
                  ...
                  <div className="absolute left-0 top-0 h-full flex flex-col justify-between text-xs font-semibold text-purple-700 pr-3">
                    <span className="bg-white/80 px-2 py-1 rounded-md shadow-sm border border-purple-200">₹{maxSales.toLocaleString()}</span>
                    <span className="bg-white/70 px-2 py-1 rounded-md shadow-sm border border-purple-200/50">₹{Math.round(maxSales * 0.8).toLocaleString()}</span>
                    <span className="bg-white/70 px-2 py-1 rounded-md shadow-sm border border-purple-200/50">₹{Math.round(maxSales * 0.6).toLocaleString()}</span>
                    <span className="bg-white/70 px-2 py-1 rounded-md shadow-sm border border-purple-200/50">₹{Math.round(maxSales * 0.4).toLocaleString()}</span>
                    <span className="bg-white/70 px-2 py-1 rounded-md shadow-sm border border-purple-200/50">₹{Math.round(maxSales * 0.2).toLocaleString()}</span>
                    <span className="bg-white/80 px-2 py-1 rounded-md shadow-sm border border-purple-200">₹0</span>
                  </div>

                  {/* Month labels */}
                  <div className="absolute bottom-0 left-0 right-0 flex justify-between px-4 pb-2">
                    {salesData.map((month, index) => (
                      <div key={month.month} className="text-xs font-semibold text-purple-700 text-center">
                        {month.month}
                      </div>
                    ))}
                  </div>
