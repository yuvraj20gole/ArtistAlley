...
interface HeaderProps {
  ...
  onNavigateToArtistDashboard: (tab?: string) => void;
  onNavigateToBuyerDashboard?: (tab?: string) => void;
  onNavigateToAdminDashboard?: (tab?: string) => void;
  currentPage: 'home' | 'login' | 'register' | 'about' | 'more' | 'bestsellers' | 'gallery' | 'artist-dashboard' | 'buyer-dashboard' | 'admin-dashboard';
  user: User | null;
  isAuthenticated: boolean;
  onLogout: () => void;
}

export function Header({ onNavigateToLogin, onNavigateToRegister, onNavigateToHome, onNavigateToAbout, onNavigateToMore, onNavigateToBestSellers, onNavigateToGallery, onNavigateToArtistDashboard, onNavigateToBuyerDashboard, onNavigateToAdminDashboard, currentPage, user, isAuthenticated, onLogout }: HeaderProps) {
    ...
    <>
      ...
      <header className="w-full border-b bg-gradient-to-r from-white/90 via-purple-50/95 to-pink-50/90 backdrop-blur-md border-purple-200/40 shadow-lg relative z-50">
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          {currentPage !== 'artist-dashboard' && currentPage !== 'buyer-dashboard' && currentPage !== 'admin-dashboard' && (
          ...
          <div className="flex items-center space-x-4">
            {isAuthenticated ? (
              <div className="hidden sm:flex items-center space-x-2">
                ...
                {user?.role === 'buyer' && onNavigateToBuyerDashboard && (
                  <Button 
                    variant="ghost" 
                    size="sm" 
                ...
                {user?.role === 'admin' && onNavigateToAdminDashboard && (
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className="text-gray-700 hover:text-purple-600 hover:bg-purple-100/50 transition-all duration-300"
          ...
          <div className="md:hidden absolute top-16 left-0 right-0 bg-gradient-to-r from-white/95 via-purple-50/95 to-pink-50/95 backdrop-blur-md border-b border-purple-200/40 shadow-xl z-50">
            <div className="container mx-auto px-4 py-4">
              <nav className="flex flex-col space-y-4">
                {/* Regular Mobile Navigation - only show on non-dashboard pages */}
                {currentPage !== 'artist-dashboard' && currentPage !== 'buyer-dashboard' && currentPage !== 'admin-dashboard' && ['Home', 'Best Sellers', 'About', 'Gallery', 'More'].map((item, index) => (
                  <a
                    key={item}
                    href="#"
                    onClick={(e) => {
                    ...
                    <>
                      ...
                      {user?.role === 'buyer' && onNavigateToBuyerDashboard && (
                        <Button 
                          ...
                          onClick={() => handleMobileNavigation(onNavigateToBuyerDashboard)}
                        >
                          My Dashboard
                        </Button>
                      )}
                      ...
                      {user?.role === 'admin' && onNavigateToAdminDashboard && (
                        <Button 
                          ...
                          size="sm" 
                          className="text-gray-700 hover:text-purple-600 hover:bg-purple-100/50 transition-all duration-300 justify-start"
                          onClick={() => handleMobileNavigation(onNavigateToAdminDashboard)}
                        >
                          Admin Dashboard
                        </Button>
                      )}
